...
//beyond //need check
	const DWORDEX checkmoneyovermaximum = (DWORDEX)pItemInfo->SellPrice * (DWORDEX)(pItemInfo->Stack ? sellItemDur : 1);
	if ( checkmoneyovermaximum > MAX_INVENTORY_MONEY)
	{
		return ERR_MAXMONEY;
	}
...
//		return 1;
	...
	//[07-02-2012] Death Added Check is over Max Money 
	DWORDEX totalprice = (DWORDEX)buyItemNum * (DWORDEX)ItemPrice;
	...
	MONEYTYPE ItemVipPoint = pItemInfo->dwBuyVipPoint;
	if(0 < ItemVipPoint)
	{
		WORD wCanBuyNum = GetCanBuyNumInVipPoint(pPlayer, buyItemNum, ItemVipPoint);
		if(wCanBuyNum < buyItemNum)
			return NOT_VIPPOINT;
	}
	//aziz Kill Shop 30 Sep
	MONEYTYPE ItemKillPoint = pItemInfo->dwBuyKillPoint;
	if(0 < ItemKillPoint)
	{
		WORD wCanBuyNum = GetCanBuyNumInKillPoint(pPlayer, buyItemNum, ItemKillPoint);
		if(wCanBuyNum < buyItemNum)
			return NOT_KILLPOINT;
	}
	//aziz Reborn Point 13 Oct
	MONEYTYPE ItemRebornPoint = pItemInfo->dwBuyRebornPoint;
	if(0 < ItemRebornPoint)
	{
		WORD wCanBuyNum = GetCanBuyNumInRebornPoint(pPlayer, buyItemNum, ItemRebornPoint);
		if(wCanBuyNum < buyItemNum)
			return NOT_REBORNPOINT;
	}
	...
	if( EI_TRUE == (rt = ObtainItemEx(pPlayer, Alloc(pPlayer, MP_ITEM, MP_ITEM_BUY_ACK, pPlayer->GetID(), DealerIdx, eLog_ItemObtainBuy, obtainItemNum, (DBResult)(BuyItemDBResult)), buyItemIdx, obtainItemNum, EmptyCellPos, EmptyCellNum, EmptyCellNum, pItemInfo->wSeal)) )
	{
		...
		MONEYTYPE addtojackpot = MONEYTYPE((ItemPrice*obtainItemNum)/10);
		//MONEYTYPE jackpot = g_pServerSystem->GetTotjackpot() + addtojackpot;
		
		//g_pServerSystem->SetMapTotaljackpot(jackpot);
		g_pServerSystem->PlusMapjackpot(addtojackpot);

		if(0 < ItemFishPoint)
		{
			//090212 pdy 낚시 포인트 버그 수정 
			//낚시 포인트 차감시 obtainItemNum(실제수령)을 곱해주지않아 다수의 아이템을 살경우 문제발생 
			DWORD dwFishPoint = DWORD(pPlayer->GetFishPoint() - (ItemFishPoint * obtainItemNum));		// * obtainItemNum를 해줘야한다. 
			pPlayer->SetFishPoint(dwFishPoint);

			// [ 23 Sep 2014 ] Death, Change MSG_DWORD to MSG_DWORD2 For Let Them Sync the VIP Point too
			...
			//let it auto update after purchase item
			...
			Log_Fishing(
				...
				DWORD ((ItemFishPoint * obtainItemNum)),		
		...
		/*if(0 < ItemVipPoint)
		{
			DWORD dwVipPoint = pPlayer->GetVipPoint() - DWORD(ItemVipPoint * obtainItemNum) ;
			pPlayer->SetVipPoint(dwVipPoint);

			MSG_DWORD msgVipPoint;
			msgVipPoint.Category = MP_VIP;
			msgVipPoint.Protocol = MP_VIP_POINT_ACK;
			msgVipPoint.dwData   = dwVipPoint;
			pPlayer->SendMsg( &msgVipPoint, sizeof(msgVipPoint) );
			//aziz let it auto update after purchase
			VipData_Update(pPlayer->GetID(), pPlayer->GetVipPoint());

			//aziz LogBuyVIP 14 Oct
			Log_Vip(
				pPlayer->GetID(),
				DWORD((ItemVipPoint * obtainItemNum)),		
				pPlayer->GetVipPoint(),
				buyItemIdx );
			//AZIZ 14 OCT Let it auto load again after purchase
			VipData_Load(pPlayer->GetID());
		} */
		//aziz Kill Shop 30 Sep
		if(0 < ItemKillPoint)
		{
			DWORD dwKillPoint = pPlayer->GetKillPoint() - DWORD(ItemKillPoint * obtainItemNum);
			pPlayer->SetKillPoint(dwKillPoint);

			MSG_DWORD msgKillPoint;
			msgKillPoint.Category = MP_KILL;
			msgKillPoint.Protocol = MP_KILL_POINT_ACK;
			msgKillPoint.dwData   = dwKillPoint;
			pPlayer->SendMsg( &msgKillPoint, sizeof(msgKillPoint) );
			//aziz let it auto update after purchase
			KillData_Update(pPlayer->GetID(), pPlayer->GetKillPoint(), 3);
		}
		//aziz Reborn Point 13 Oct
		/*if(0 < ItemRebornPoint)
		{
			DWORD dwRebornPoint = pPlayer->GetRebornPoint() - DWORD(ItemRebornPoint * obtainItemNum);
			pPlayer->SetRebornPoint(dwRebornPoint);

			MSG_DWORD msgRebornPoint;
			msgRebornPoint.Category = MP_UTILITY;
			msgRebornPoint.Protocol = MP_REBORNPOINT_ACK;
			msgRebornPoint.dwData   = dwRebornPoint;
			pPlayer->SendMsg( &msgRebornPoint, sizeof(msgRebornPoint) );
			//aziz let it auto update after purchase
			RebornPoint_Update(pPlayer->GetID(), pPlayer->GetRebornPoint());
		} */
	}
...
eItemUse_Err CItemManager::UseItem(CPlayer* pPlayer, POSTYPE TargetPos, DWORD wItemIdx)
{
	...
	{
		if( SIEGEWARFAREMGR->IsFobiddenItemTournament(wItemIdx) ) return eItemUseErr_SiegeWarfareForbiddenItem;
	}
	...
	case ITEM_KIND_PKPOINT_UP:
		{
			// [ 9 Dec 2013 ] Death , RollBack Code to Original separate Point item to new
	...
	// Death Separate PK and Point Item Kind
	case ITEM_KIND_POINT_UP:
		{
			// Death Comment if for we use this Item Kind for Point PK
			POINTTYPE addPoint = pItemInfo->SupplyValue;
			AddCharacterPoint( pPlayer->GetID(), addPoint );			
		}
		break;
	
	//aziz Reborn Point 13 Oct
	case ITEM_KIND_REBORNPOINT_UP:
		{
			DWORD addRBPoint = pItemInfo->SupplyValue;
			pPlayer->SetRebornPoint( pPlayer->GetRebornPoint() + addRBPoint );
			...
			msgRebornPoint.Category = MP_UTILITY;
			msgRebornPoint.Protocol = MP_REBORNPOINT_ACK;
			msgRebornPoint.dwData	= pPlayer->GetRebornPoint();
			pPlayer->SendMsg(&msgRebornPoint, sizeof(msgRebornPoint));
		}
		break;
	...
	// [27 - Mar - 2012] Death add Function For Support Increse HP MP By Percent
	case ITEM_KIND_MP_RECOVER_PERCENT:	
		{			
			// Cal Mana Value
			DWORD MaxValMana = pPlayer->GetMaxMana();
			DWORD InceseValueMana = MaxValMana * pItemInfo->SupplyValue / 100;

			CCharacterCalcManager::StartUpdateMana( pPlayer, InceseValueMana);

			bEffectKind = 2;//내력 회복 이펙트
			break;
		}
	case ITEM_KIND_HPMP_RECOVER_PERCENT:		//동시회복제
		{
			// Cal HP Value
			DWORD MaxValHP = pPlayer->GetMaxLife();
			DWORD InceseValueHP = MaxValHP * pItemInfo->SupplyValue / 100;
			// Cal Mana Value
			DWORD MaxValMana = pPlayer->GetMaxMana();
			DWORD InceseValueMana = MaxValMana * pItemInfo->SupplyValue / 100;

			CCharacterCalcManager::StartUpdateLife( pPlayer, InceseValueHP );
			CCharacterCalcManager::StartUpdateMana( pPlayer, InceseValueMana );
				bEffectKind = 3;
			break;
		}
	...
	case ITEM_KIND_RETURN_SCROLL:
		{
			//---KES PK Mode 080104
			//---KES AUTONOTE
			if( (pPlayer->IsPKMode() && g_pServerSystem->GetMapNum() != PVP) || pPlayer->GetAutoNoteIdx() || g_pServerSystem->GetMapNum() == StreetTournament)
			{
				return eItemUseErr_Err;
			}
			//---------------------
...
eLogitemmoney CItemManager::Reinforce( CPlayer* player, const MSG_ITEM_REINFORCE_SYN& input, const CBackupSlot* backupSlot )
{
		...
		++it )
	{
		...
		else if( 0 < ( rand() % 101 - sourceItemInfo->LimitLevel / 4 ) )//aziz give 50% for lv200 equip
		{	
...
MP_PROTOCOL_ITEM CItemManager::Enchant(CPlayer* player, const MSG_ITEM_ENCHANT_SYN* message, eLogitemmoney& log )
{
	...
	// 성공 여부 판정
	{
		...
		if(0 == basis)
		{
			...
			basis = 1000.0f * float(pow( basis, option.mEnchant.mLevel ));
		}
	...
	MONEYTYPE addtojackpot = MONEYTYPE(enchantCost/10);
	//MONEYTYPE jackpot = g_pServerSystem->GetTotjackpot() + addtojackpot;
	//g_pServerSystem->SetMapTotaljackpot(jackpot);
	g_pServerSystem->PlusMapjackpot(addtojackpot);
	...
	//aziz change 10 to 14
	if( option.mEnchant.mLevel >= 14 )
	{
		MSG_NAME_DWORD3 msg;
		ZeroMemory( &msg, sizeof(msg) );
		msg.Category		= MP_ITEM;
		...
		if(eEnchantResultNotify_Success)
		{
			char buffname[MAX_NAME_LENGTH+1] = "";
			char buffitemname[MAX_ITEMNAME_LENGTH+1] = "";
			SafeStrCpy( buffname, player->GetObjectName(), MAX_NAME_LENGTH+1 );
			SafeStrCpy( buffitemname, sourceInfo->ItemName, MAX_ITEMNAME_LENGTH+1 );

			SYSTEMTIME sysTime;
			GetLocalTime( &sysTime );
			FILE* fpLog = NULL;
			char fname[256];
			sprintf(fname,"./webstream/event_month%02ddate%02d.txt", sysTime.wMonth, sysTime.wDay);
			fpLog = fopen( fname, "a+" );
			if( fpLog )
			{
				fprintf( fpLog, "<font color='green'>[%02d:%02d][Enchant]: Congratulations! <b>%s</b> has successfuly enchant <b>%s</b> to <b>+%d</b></font>\n", sysTime.wHour, sysTime.wMinute, buffname, buffitemname, option.mEnchant.mLevel );
				fclose( fpLog );
			}
	...
	if (m_stChangeItemInfo)
	{
		for(int i=0; i<m_nChangeItemInfoNum; i++)
		{
			if (nItemID == (DWORD)m_stChangeItemInfo[i].nChangeItemID)
			{
				...
				DWORD nItemIDnya = nItemID;
				//char nItemNamenya[MAX_NAME_LENGTH+1] = pstItem->ItemName;
				char buffitemname[MAX_ITEMNAME_LENGTH+1] = "";
				SafeStrCpy( buffitemname, pstItem->ItemName, MAX_ITEMNAME_LENGTH+1 );
						...
						case TRK_ITEM:
							{
									...
									k == m_stChangeItemInfo[i].stResult[j].nResultBaseNum-1)
								{
									...
									if(IsChangeItemWithNotify(nItemID))
									{
										MSG_USER_ADD_ITEM_BOX message;
										ZeroMemory(&message, sizeof(message));
										message.Category = MP_SIGNAL;
										message.Protocol = MP_SIGNAL_USER_ADD_ITEM_BOX;
										SafeStrCpy(message.CharacterName, pPlayer->GetObjectName(), sizeof(message.CharacterName) / sizeof(*message.CharacterName));
										message.dwIconIdx1 = nItemIDnya;
										message.dwIconIdx2 = nItemID;
										message.dwNum		= WORD(nItemNum);
										PACKEDDATA_OBJ->QuickSend(pPlayer, &message, sizeof(message));
										
										ITEM_INFO* pstItem2 = GetItemInfo(nItemID);
										if (pstItem2 == NULL)
										{
											goto NEXT_RESULT;
										}
										...
										char buffname[MAX_NAME_LENGTH+1] = "";
										char buffitemname2[MAX_ITEMNAME_LENGTH+1] = "";
										SafeStrCpy( buffname, pPlayer->GetObjectName(), MAX_NAME_LENGTH+1 );
										SafeStrCpy( buffitemname2, pstItem2->ItemName, MAX_ITEMNAME_LENGTH+1 );
										...
										if( fpLog )
										{
											fprintf( fpLog, "<font color='green'>[%02d:%02d][ITEM]: Grats! <b>%s</b> Open %s get %d <b>%s</b></font>\n", sysTime.wHour, sysTime.wMinute, buffname, buffitemname, nItemNum, buffitemname2 );
											fclose( fpLog );
										}
									}
...
void CItemManager::SetItemInfo(DWORD ItemIdx,ITEM_INFO* pInfo,CMHFile* pFile)
{
	...
	//aziz MallShop in Game Method 1
	pInfo->dwBuyVipPoint = pFile->GetDword();
	//aziz Kill Shop 30 Sep
	pInfo->dwBuyKillPoint = pFile->GetDword();
	//aziz Reborn Point 13 Oct
	pInfo->dwBuyRebornPoint = pFile->GetDword();
}
...
{	
	...
	case MP_ITEM_MOVE_SYN:
		{
			...
			//hack check
			if (pmsg->ToPos >= (TP_WEAR_START - 1) && pmsg->ToPos <= (TP_WEAR_END - 1)) //check only for equip slot
			{
				WORD tempPart;
				eEquipType tempType;
	...
	case MP_ITEM_ENCHANT_SYN:
		{
			...
			// [ 08 - May - 2013 ] Death Recheck ItemIdx is Enchance Item [ Protect Cheat Engine Hacked]
			if (m->mProtectionItem.wIconIdx != 0)
			{
				CItemSlot* pEnhanceterInventory	= player->GetSlot( eItemTable_Inventory );
				const ITEMBASE* pRealItem = pEnhanceterInventory->GetItemInfoAbs( m->mProtectionItem.Position );
				if( pRealItem->wIconIdx != m->mProtectionItem.wIconIdx )
				{
					return;
				}
			}
	...
	case MP_ITEM_MAPMOVE_SCROLL_SYN:
		{
			...
			//---KES PK Mode 080104
			//---KES AUTONOTE
			if( (pPlayer->IsPKMode() && g_pServerSystem->GetMapNum() != PVP) || pPlayer->GetAutoNoteIdx() )
			{
				MSG_DWORD msg;
				msg.Category	= MP_ITEM;
			...
			//if ( typepre == 1 && pPlayer->GetVIPLevel() < 1)
			if ( typepre == 1 ) //free for all
			{
				if( (pPlayer->IsPKMode() && g_pServerSystem->GetMapNum() != PVP) || pPlayer->GetAutoNoteIdx() )
				{
					MSG_DWORD msg;
	...
	case MP_ITEM_APPLY_OPTION_SYN:
		...
		case MP_ITEM_SHOPITEM_SHOUT_SYN:
		{
			...
			SafeStrCpy(msgforServer.Msg, pmsg->Msg, sizeof(pmsg->Msg) / sizeof(*pmsg->Msg));
			SafeStrCpy(msgforServer.Name, pmsg->Name, sizeof(pmsg->Name) / sizeof(*pmsg->Name));
...
void CItemManager::LoadDissolveScript()
{
	...
	{
		ASSERTMSG( 0, "Failed to Load itemDissolve.bin" );
		return;
	}
	...
	while( ! file.IsEOF() )
	{
		...
		// 그리고 결과를 얻을 때는 랜덤값을 얻은 후 맵의 lower_bound()로 해당되는 값을 찾아낸다.
		{
			...
			{
				ASSERT( 0 && "The sum of the probability values in the decomposition script is not 10000. We can not guarantee the correct result" );
			}
		}
	}
}
...
BOOL CItemManager::IsRare( const ITEM_INFO& info ) const
{
	...
	case ITEM_INFO::eKindRebirthExchange:
		{
			return TRUE;
		}
	}
...
//	DB_UpdateToDefaultStats( pPlayer->GetID(), Str, Dex, Vit, Int, Wis ) ;
//	
//	//aziz Additional stats 23 nov 2014
//	//AStats_Point( pPlayer->GetID() ) ;
...
//aziz MallShop in Game Method 1
WORD CItemManager::GetCanBuyNumInVipPoint(CPlayer* pPlayer, WORD buyNum, MONEYTYPE Point)
{
	MONEYTYPE PlayerPoint = (MONEYTYPE)pPlayer->GetVipPoint();
...
//aziz Reborn in Game 29 Sep
BOOL CItemManager::UseItem_ResetLevel(CPlayer* pPlayer)
{
	if( !pPlayer )
	{
		return FALSE;
	...
	if( pPlayer->IsPKMode() )
		return FALSE;
	pPlayer->SetResetLevel(true);
	//DWORD dwLevel = 1;
	//DWORD dwRBPoint = 1;
	DB_ResetToDefaultLevel( pPlayer->GetID() ) ;
	//pPlayer->SetResetLevel(false);

	return TRUE;
}
//UseItem_ResetStats New
BOOL CItemManager::UseItem_ResetStats(CPlayer* pPlayer)
{
	if( !pPlayer )
	{
...
//aziz Kill Shop 30 Sep
WORD CItemManager::GetCanBuyNumInKillPoint(CPlayer* pPlayer, WORD buyNum, MONEYTYPE Point)
{
	MONEYTYPE PlayerPoint = (MONEYTYPE)pPlayer->GetKillPoint();
...
//aziz Reborn Point 13 Oct
WORD CItemManager::GetCanBuyNumInRebornPoint(CPlayer* pPlayer, WORD buyNum, MONEYTYPE Point)
{
	MONEYTYPE PlayerPoint = (MONEYTYPE)pPlayer->GetRebornPoint();
...
void CItemManager::LoadChangeitemNotify()
{
	CMHFile fpx;
	char szFile[256] = {0,};
	char szLine[256] = {0,};

	sprintf(szFile, "./System/Resource/notifychangeitem.bin");
	fpx.Init(szFile, "rb");

	while(FALSE == fpx.IsEOF())
	{
		fpx.GetLine(szLine, sizeof(szLine));
		if ( strstr(szLine, "//") ||
			IsEmptyLine(szLine))
		{
			continue;
		...
		m_addnotifychangeitem.push_back(itemIdx);
	}

	fpx.Release();
}

BOOL CItemManager::IsChangeItemWithNotify(DWORD ItemIdx)
{
	std::list<DWORD>::iterator BeginIter = m_addnotifychangeitem.begin();

	while( BeginIter != m_addnotifychangeitem.end() )
	{
...
ITEM_OPTION CItemManager::LimiterItemOption(ITEM_OPTION option, const ITEM_INFO* info, CPlayer* player, BOOL isRare)
{
	...
	DWORD maxreinint = 40;
	DWORD maxreinwis = 40;
	DWORD maxreinlife = 2000;
	DWORD maxreinmana = 2000;
	DWORD maxreinliferec = 40;
	DWORD maxreinmanarec = 40;
	DWORD maxreinpatk = 30;
	DWORD maxreinmatk = 30;
	DWORD maxreinpdef = 30;
	DWORD maxreinmdef = 30;
	DWORD maxreinmspeed = 150;
	DWORD maxreineva = 30;
	DWORD maxreinacc = 30;
	DWORD maxreincrit = 150;
	...
	if(m_logged)
	{
		...
		FILE* fpLog = NULL;
		char fname[256];
		sprintf(fname,"./log/itemlimiter/loglimiter%02d-%02d.txt", sysTime.wMonth, sysTime.wDay);
		fpLog = fopen( fname, "a+" );
		if( fpLog )
		{
			fprintf( fpLog, "[%02d:%02d][%s][%d - %s]: before[%d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d] after[%d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d] enchantbonus[%d * %d]\n", sysTime.wHour, sysTime.wMinute, player->GetObjectName(), option.mItemDbIndex, info->ItemName,
...
ITEM_OPTION CItemManager::LoadLimiterItemOption(ITEM_OPTION option, const ITEM_INFO* info, CPlayer* player, BOOL isRare)
{
	...
	if(m_logged)
	{
		...
		FILE* fpLog = NULL;
		char fname[256];
		sprintf(fname,"./log/itemlimiter/logloadlimiter%02d-%02d.txt", sysTime.wMonth, sysTime.wDay);
		fpLog = fopen( fname, "a+" );
		if( fpLog )
		{
			fprintf( fpLog, "[%02d:%02d][%s][%d - %s]: before[%d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d] after[%d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d - %d] enchantbonus[%d * %d]\n", sysTime.wHour, sysTime.wMinute, player->GetObjectName(), option.mItemDbIndex, info->ItemName,
