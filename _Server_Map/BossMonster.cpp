...
{
	CMonster::Process();	//  ÀÚµ¿¼Ò¸êÀ» È®ÀÎÇÏ±â À§ÇØ ¸ÕÀúÈ£ÃâÇÑ´Ù.
...
{	
	...
	////////////////////////////////////////////////////////////////////////////////
	/// 06. 08. 2Â÷ º¸½º - ÀÌ¿µÁØ
	/// º¸½ºÀÇ Å¸°ÙÆÃ ¹æ½ÄÀÌ ¸ÅÈ¸ ·£´ý °áÁ¤ÀÏ °æ¿ì Å¸°ÙÀ» »õ·Î Àâ´Â´Ù
...
void CBossMonster::Stand()
{
	...
	if(m_BossState.IsStateFirst())
	{
		...
		// 080205 KTH -- ¸Ê¿¡ Ãæµ¹Ã¼Å©°¡ ÇØÁöµÇ¾î ÀÖÀ¸¸é ´Ù½Ã ÃÊ±âÈ­ ½ÃÄÑÁØ´Ù.
		if( IsNoCheckCollision() )
		{
			VECTOR3 vPos;
			GetPosition( &vPos );
...
void CBossMonster::OnLifeEvent(DWORD Life)
{
	...
	for(int i = 0; i< m_pBossMonsterInfo->GetMaxEventStateNum(); ++i)
	{
		if(m_BossEventState[i].Condition == eBOSSCONDITION_LIFE)
		{
			...
			{
				// 091028 ShinJS --- DWORD Overflow Çö»ó ¼öÁ¤
...
void CBossMonster::Delete()
{
	...
	// ¸®Á¨ Á¤º¸ µî·Ï 080204 KTH -- ÀâÁö ¸øÇÏ¿´À» °æ¿ì ¸®Á¨ ½Ã°£À» 1½Ã°£À¸·Î ¸¶Ãá´Ù.
