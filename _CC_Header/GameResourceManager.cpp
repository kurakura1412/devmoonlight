...
{
	CTargetData mna;
	DWORD aaCTargetData = sizeof(mna);
	//DWORD aaCTargetDataaddre = (DWORD)&(mna.TargetData[0]) - (DWORD)&(mna);
	(void)aaCTargetData;
	//(void)aaCTargetDataaddre;
...
BOOL CGameResourceManager::LoadPlayerxMonsterPoint()
{
	...
	while(1)
	{
		...
		for(int levelgap = 0 ; 
		levelgap < MAX_MONSTERLEVELPOINTRESTRICT_NUM + MONSTERLEVELRESTRICT_LOWSTARTNUM + 1 ; 
		levelgap++)
		{
