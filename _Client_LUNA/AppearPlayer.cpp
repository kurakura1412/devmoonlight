...
//================================== SCRAF  ! copy as it !
	m_pEngineObject->ChangePart(		eAppearPart_Scarf,		"NULL.MOD");
//================================== ESCRAF 
	// 결과 리턴.
	return TRUE ;
	...
	for(int n=0 ;n < eWearedItem_Max ; ++n)
	{
		if (/*m_pPlayer->GetChangePart(n) && */WearedItemIdx[n] && !HideParts[n])
		{
			...
			case ePartType_Head_CatEar:
				...
				break;
//================================== SCRAF  ! copy as it !
      case ePartType_Special_Scarf:
				{
          m_pEngineObject->ChangePart( eAppearPart_Scarf, pModList->ModFile[ PartModelNum ] ) ; // replace m_ModelList at  eAppearPart_Scarf !
					m_stShowFlag.IsShowFace		= TRUE ;
					m_stShowFlag.IsShowGlasses  = TRUE ;
					m_stShowFlag.IsShowHelmet	= TRUE ;
					m_stShowFlag.IsShowMask		= TRUE ;
...
//================================== SCRAF
      case ePartType_Special_Scarf :
				{
          // unused -->
					//CEngineObject* pScarf = new CEngineObject ;
					//pScarf->Init(pModList->ModFile[PartModelNum],NULL,eEngineObjectType_Weapon) ;
					//m_pEngineObject->AttachDress(pScarf,"Bip01 Head") ;
				}
				break;
//================================== ESCRAF
			case ePartType_Head_Mask :
